...
function handleLogoSelect(event) {
  select(event, logoIcon);
}

//task bar events
taskBar.addEventListener("click", handleTaskBar);
function handleTaskBar(event) {
  alert("ADMIN PERMISSIONS NEEDED");
}

//hornet icon events
hornetIcon.addEventListener("click", handleHornetSelect);
function handleHornetSelect(event) {
  select(event, hornetIcon);
}
hornetIcon.addEventListener("dblclick", handleHornet);
function handleHornet(event) {
  alert("Shaw !");
}
...
function insertLink() {
  instTag.insertAdjacentHTML(
    "beforeend",
    `<a href="./frontend/pages/intro.html" id="link"> >START GAME? </a>`,
  );
}
